...
class MotionPlanning(Drone):
    ...
    def plan_path(self):
        ...
        local_goal = global_to_local((-122.396311, 37.793685,  -TARGET_ALTITUDE), self.global_home) #FEDEX
    ...
    def local_path_planning(self):
        ...
        (in_collision, local_path, collision_point) = local_path_obstacle_detection(self.voxmap, local_target)
        ...
        if in_collision:
            ...
            while in_collision:
                ...
                print('=============')
                print('collision detected, replanning path')
                print('angle: ',angle)
            ...
            print('collision: change the plan')

            #if the angle is non-zero
            if angle != 0 and not in_collision:

                new_waypoint = [
                    int(self.local_position[0] - int(self.voxmap.shape[0]/2) + local_path[-1][0]),  
                    int(self.local_position[1] - int(self.voxmap.shape[1]/2) + local_path[-1][1]), 
