...
def update_voxmap(voxmap, local_position, sampler, voxel_size=1):
    ...
    alt_min =  local_position[2] - voxel_size * zd / 2
    alt_max =  local_position[2] + voxel_size * zd / 2

    # print('local_position ',local_position)
    # print('north_min ',north_min)
    # print('north_max ',north_max)
    ...
    #center voxel
    center = (int(xd/2), int(yd/2), int(zd/2))

    #maximum distance between point and outer voxels
    d_voxmap = np.sqrt(((xd/2)**2+(yd/2)**2) + ((zd/2)/2)**2)
...
def check_obstacle(voxmap, v, voxel_size=1):
    ...
    goal = ( int(center[0]+v[0]), int(center[1]+v[1]), int(center[2]+v[2]) )
    ...
    for p in pts:
        ...
        if p[0]>=0 and p[0]<xd and p[1]>=0 and p[1]<yd:

            if voxmap[p[0], p[1], center[2]]:
                return True
