...
class MotionPlanning(Drone):
    ...
    def plan_path(self):
        ...
        for i in range(len(path)-1):
            ...
            #current waypoint
            p1 = np.array([path[i][0] + north_offset, 
                  path[i][1] + east_offset])

            #next waypoint
            p2 = np.array([path[i+1][0] + north_offset, 
                  path[i+1][1] + east_offset])
            ...
            waypoints.append([p1[0], p1[1], TARGET_ALTITUDE, angle])

        #append last waypoint
        waypoints.append([p2[0], p2[1], TARGET_ALTITUDE, angle])
